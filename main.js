...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const previewsBg = document.querySelector(".preview-bg");
    ...
    function changeBg(newImgsrc) {
        ...
        previewsBg.appendChild(newImg);

        gsap.to(newImg, { opacity: 1, duration: 0.5 });

        if (previewsBg.children.length > 1) {
            const oldImg = previewsBg.children[0]; // Ensure you're targeting the first child
            if (oldImg && previewsBg.contains(oldImg)) {
                // Check if oldImg exists and is a child of previewsBg
                gsap.to(oldImg, {
                    opacity: 0,
                    duration: 0.5,
                    onComplete: () => {
                        if (previewsBg.contains(oldImg)) {
                            // Double-check before removing
                            previewsBg.removeChild(oldImg);
                        }
                    },
                });
            }
    ...
    previews.forEach((preview, index) => {
      ...
      index + 1
        ...
        previewElement.innerHTML = `
            <div class="preview-img"><img src="${preview.img}" alt="${preview.title}" /></div>
    ...
    items.forEach((item, index) => {
        item.addEventListener("mouseenter", () => {
            ...
            const newBg = `./assets/img-${index + 1}.jpg`;
            changeBg(newBg);
